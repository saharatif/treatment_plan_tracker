...
OUT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "docs", "Marcus_Elliot_Treatment_Plan_v2.pdf")
W, H = letter
...
# Bottom-of-page safe margin (above the footer rule)
BOTTOM = 55
...
def wrap(c, x, y, mw, s, font="Helvetica", size=9, col=DARK, lh=13):
    ...
    for w_ in words:
        t = (line_ + " " + w_).strip()
        if stringWidth(t, font, size) <= mw: line_ = t
        else:
...
def wrap_h(mw, s, font="Helvetica", size=9, lh=13):
    """Compute the total height wrapped text will occupy (no canvas needed)."""
...
# ═══════════════════════════════════════════════════════════════════════════
# The 10 Orbs — each maps to a catalog entry in app.orbs (see
# agents/01-foundations-and-storage.md, Step 4: Orb Catalog)
# ═══════════════════════════════════════════════════════════════════════════

ORBS = [
    {
        "num": "Orb 1", "title": "Blood Work — Baseline Labs",
        "catalog": "LAB-01", "code": "CPT: 83036, 80053",
    ...
    },
    {
        ...
        "catalog": "MED-01", "code": "Rx: Metformin HCl 500mg  |  ICD-10: E11.9",
    ...
    },
    {
        ...
        "catalog": "MED-02", "code": "Rx: Lisinopril 10mg  |  ICD-10: I10",
        "dates": "Target Date: June 13, 2025 (Day One)",
    ...
    },
    {
        ...
        "catalog": "VIT-01", "code": "OTC Supplements",
        "dates": "Target Date: June 13, 2025 (Day One)",
    ...
    },
    {
        ...
        "catalog": "EXR-01", "code": "Exercise — Aerobic",
    ...
    },
    {
        ...
        "catalog": "EXR-02", "code": "Exercise — Aerobic / Resistance",
    ...
    },
    {
        ...
        "catalog": "MON-01", "code": "HCPCS: A4253",
    ...
    },
    {
        ...
        "catalog": "DIE-01", "code": "Diet / Nutrition",
    ...
    },
    {
        ...
        "catalog": "REF-01", "code": "ICD-10: Z01.01, Z01.89",
...
# ═══════════════════════════════════════════════════════════════════════════
# Block height / draw functions — each block's draw_*() consumes exactly the
# height its *_height() reports, so pagination can be computed ahead of time.
# ═══════════════════════════════════════════════════════════════════════════

ORBS_TITLE_HEIGHT = 16 + 14  # title row + hline gap, drawn once at top of page 2

def orb_height(orb):
    h = 14   # heading row
    h += 11  # catalog ref line
    h += 12  # "Short Term Goal / Objective:" label
    h += wrap_h(TW - 8, orb["goal"], "Helvetica", 9, 12)
    h += 2
    h += 13  # dates / completion / status row
    h += 12  # "Intervention:" label
    for intv in orb["interventions"]:
        h += wrap_h(TW - 18, intv, "Helvetica", 9, 12)
    h += 13  # frequency / duration / progress row
    h += 12  # divider rule + trailing gap
    return h

def draw_orb(cv, y, orb):
    ...
    txt(cv, LM, y, f"Catalog Ref: {orb['catalog']} (app.orbs)", "Helvetica", 7.5, GREY)
    y -= 11
...
def signature_height():
    h = 16   # hline gap
    h += 12  # "STATUS:" label
    h += wrap_h(TW, STATUS_TEXT, "Helvetica", 9, 13)
    h += 14  # gap before signature lines
    h += 12 + 11 + 11  # name row, clinic/ack row, date row
    return h

def draw_signatures(cv, y):
    hline(cv, y, BLACK, 0.8)
    y -= 16
...
# Blocks rendered after page 1, in order. Each is (height_fn, draw_fn).
CONTENT_BLOCKS = (
    [(lambda o=o: orb_height(o), (lambda cv, y, o=o: draw_orb(cv, y, o))) for o in ORBS]
    + [
        (metrics_height, draw_metrics),
        (snap_height, draw_snap),
        (barriers_height, draw_barriers),
        (signature_height, draw_signatures),
    ]
)

def compute_total_pages():
    """Dry run: simulate pagination of CONTENT_BLOCKS to count pages after page 1."""
    content_pages = 1  # page 2 always exists (it carries the orbs title)
    y = (H - 52) - ORBS_TITLE_HEIGHT
    for height_fn, _ in CONTENT_BLOCKS:
        h = height_fn()
        if y - h < BOTTOM:
            content_pages += 1
            y = H - 52
        y -= h
    return 1 + content_pages

def build():
    cv = C.Canvas(OUT, pagesize=letter)
    cv.setTitle("Outpatient Treatment Plan — Marcus J. Elliot")

    TOTAL = compute_total_pages()
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # PAGE 2+ — The 10 Orbs, Success Metrics, SNAP, Barriers, Signatures.
    # Pagination is dynamic: every block is measured before drawing, so no
    # orb (and no trailing section) is ever silently dropped off a page.
    # ══════════════════════════════════════════════════════════════════════════
    pg = 2
    y = pg_hdr(cv, pg, TOTAL)
    pg_ftr(cv, pg, TOTAL)
    ...
    for height_fn, draw_fn in CONTENT_BLOCKS:
        h = height_fn()
        if y - h < BOTTOM:
            cv.showPage()
            pg += 1
    ...
    print(f"Done: {OUT}  ({sz//1024} KB)  {TOTAL} pages")
